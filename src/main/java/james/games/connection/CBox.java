...
public abstract class CBox extends BaseMap {
	...
	public CBox(CMap map, int playerNum,boolean save,CAction[] gameActions) {
		...
		bar.add(view);
		setMenuBar(bar);
		setMiniTitle("Pl: "+(this.player+1)+"  "+(side ? "sky blue: top to bottom" : "purple: left to right"));
	}
	...
	public void proccessAction2(CAction action) {
		...
		else if (clazz==DeclareVictory.class) { testVictory((DeclareVictory) action); }
	...
	public void turn() {
		if (!finiteDeclares) testVictory(true);
		if (!finiteDeclares) testVictory(false);
	...
	public int testVictory(boolean otherside) {
            if (end) return 0;
            //JOptionPane.showMessageDialog(this,"test victory: myside = "+side+" testing = "+otherside);
		Pathfinder finder = new Pathfinder(board(),new SquareCondition() {
			public boolean conditionFulfilled(Square square) {
                            //System.out.println(Square.coordinates(square.X,square.Y)+":"+square.player()+" "+square.isPlayer(otherside));
				return square.isPlayer(otherside);
			}
		});
		boolean win = findPath(finder,otherside);
		if (win) {
                    //JOptionPane.showMessageDialog(this,"found path");
                    end=true;
			if (side==otherside) win();
                        else loss();
			return 1;
		}
		else if (finiteDeclares&&--declaresLeft<=0) {
                    //JOptionPane.showMessageDialog(this,"no path");
                    end=true;
			if (side!=otherside) win();
                        else loss();
			return -1;
	...
	public void testVictory(DeclareVictory action) {
            boolean otherside;
            if (action.player()==player) otherside=side;
            else otherside=!side;
		switch (testVictory(otherside)) {
			case -1:
	...
	private boolean findPath(Pathfinder finder, boolean otherside) {
		if (otherside) return finder.path(lineArray(width()),constArray(0,width()),lineArray(width()),constArray(height()-1,width()));
	...
	private void updateTitle() {
		...
		else setTitle("pl: "+(player+1)+" (waiting)");
	}
